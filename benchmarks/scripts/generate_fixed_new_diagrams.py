...
def draw_telemetry_model():
    ...
    ax.text(0.25, 0.72, 'time.Ticker(2s)\nHTTP: Prometheus DCGM\nHTTP: Carbon API', ha='center', va='center', fontsize=10)
    ...
    os.makedirs('docs/new_diagrams', exist_ok=True)
    plt.savefig('docs/new_diagrams/telemetry_goroutine_model.png', bbox_inches='tight', dpi=300)
    plt.close()
...
def draw_kv_cache_topology():
    ...
    plt.savefig('docs/new_diagrams/kv_cache_topology.png', bbox_inches='tight', dpi=300)
    plt.close()

if __name__ == "__main__":
    draw_telemetry_model()
    draw_kv_cache_topology()
    print("Diagrams generated successfully in docs/new_diagrams.")
